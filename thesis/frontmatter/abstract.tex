...
oracle's sorted output. Transactional sibling-list updates, checked state
audits, deterministic replay, and fixed execution policies separate
correctness instrumentation from the pre-certified \texttt{minimal} call used
for timing. A frozen formal experiment evaluates 60 controlled cases at five
sizes from $n=32$ to $n=512$, comparing Python sort, a complete oracle-backed
reference pipeline, and the ordinary-list paper reconstruction.

All 3600 measured rows in the formal run have correct output and no recorded
error, and all 60 untimed checked audits pass. The median exact-case
paper/reference runtime ratio decreases from $3.226$ at $n=32$ to $0.567$ at
