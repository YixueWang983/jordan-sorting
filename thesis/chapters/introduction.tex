...
Jordan sorting lies at the intersection of computational geometry, algorithm
design, and data-structure engineering. More generally, computational
geometry studies how geometric restrictions can be converted into algorithmic
structure and supported by purpose-built data structures
\parencite{deberg2008computational}. This setting makes Jordan sorting a useful
case study in the difference between an abstract algorithmic result and the
costs of a concrete representation.
...
This combination of reconstruction, implementation, and controlled evaluation
follows the algorithm-engineering view that design, analysis, implementation,
and experimentation are related but distinct parts of algorithmic research
\parencite{sanders2009engineering}. The experimental component is likewise
treated as a source of bounded evidence about implemented algorithms, not as a
replacement for theoretical analysis \parencite{mcgeoch2012guide}.
...
The thesis considers two deliberately different research implementations. The
...
\begin{enumerate}[label=\textbf{RQ\arabic*:},leftmargin=*]
  ...
  \item \textbf{Observed runtime behavior.} Across the pre-specified input sizes and
...
The first question is addressed through focused tests, bounded exhaustive
validation over oracle-certified cases, checked-state audits, and deterministic
replay. The second uses the formal experiment under its pre-specified protocol.
...
\begin{enumerate}
  ...
  \item It establishes a reproducible empirical workflow with pre-specified
    cases, documented timing scopes, archived manifest-bound evidence, and a
    separate fail-closed evidence-contract validator. The resulting study
    compares Python sort, the oracle-backed reference pipeline, and the paper
    ordinary-list call without treating their scopes as interchangeable.
\end{enumerate}
...
All 3600 measured rows in the formal experiment belong to 60 exact cases that
each passed one untimed checked-state audit before timing. Every row records an
oracle-certified input, correct output, and no recorded error. All 180
case-algorithm summaries are correct and error-free, and all 60 case audits
pass. This is empirical correctness evidence for the pre-specified cases, not a proof
for every Jordan sequence.
...
\Cref{chap:methodology} specifies the pre-specified valid-input experiment, timing
