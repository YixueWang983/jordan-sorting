...
| ID | Permitted thesis claim | Primary evidence locator | Required boundary |
| --- | --- | --- | --- |
| R-01 | All 3,600 measured rows have valid oracle certification, correct output, passed audit, and no recorded error; all 180 case summaries and 60 audits pass. | [`week12_correctness_audit_totals.csv`](../analysis/week12_correctness_audit_totals.csv): all failure-count fields equal zero; [`week12_live_validation_report.json`](../analysis/week12_live_validation_report.json): `valid=true` | This is empirical correctness over the frozen cases, not a proof for every Jordan sequence. |
